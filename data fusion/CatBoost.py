...
parser = argparse.ArgumentParser(description="CatBoost微生物属分类融合训练脚本")
parser.add_argument("--train", required=True)
parser.add_argument("--test", required=True)
parser.add_argument("--val", required=True)
parser.add_argument("--output", default="CatBoost")
...
for i in range(REPEAT):
    seed = i
    clf = CatBoostClassifier(iterations=500, learning_rate=0.05, random_state=seed, verbose=0)
...
print(f"✅ CatBoost 完成，结果已保存到 {OUTPUT_DIR}")
